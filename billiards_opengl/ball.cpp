...
void Ball::move_vec() {
	for (size_t i = 0; i < array_length(vec)-1; i++) {
		vec[i] = vec[i] * friction; //attenuation vector
		if (abs(vec[i]) < vec_threshold) vec[i] = 0.0; //vector stall
	}
	for (size_t i = 0; i < array_length(vec); i++) pos[i] += vec[i];
}
...
void Ball::move_pos(GLdouble add_pos[]) {
	for (size_t i = 0; i < array_length(pos); i++) pos[i] += add_pos[i];
}

void Ball::add_force(GLdouble add_vec[]) {
	for (size_t i = 0; i < array_length(vec); i++) vec[i] += add_vec[i];
}
