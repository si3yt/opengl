	...
	for (size_t i = 0; i < array_length(balls); i++) {
		if (!collision.bottom_floor(balls[i].pos)) {
			...
			if (balls[i].vec[2] == 0.0) {
				...
				if (collision.wall_tbside_judge(balls[i].pos[1])) {
			...
			else balls[i].move_vec();
			balls[i].draw(tex_id[i]);
		//resurrection player ball
		} else {
...
bool ObjectControl::ball_all_stop() {
	for (size_t i = 0; i < _BALL_NUM; i++) {
		if (!collision.bottom_floor(balls[i].pos)) {
			if (balls[i].vec[0] != 0.0 || balls[i].vec[1] != 0.0 || balls[i].vec[2] != 0.0) return false;
		}
	}
	return true;
}
...
void ObjectControl::texture_init() {
	...
	for (size_t i = 0; i < _BALL_NUM; i++) tex_id[i] = texture.create_texture(tex_id[i], const_cast<char*>(_BALL_TEXTURE[i]));
}
