...
bool Collision::floor_judge(GLdouble pos[]) {
	...
	if (floor_height / 2 > abs(pos[1]) && floor_width / 2 - hole_size + _BALL_RADIUS / 2 > abs(pos[0]) && ball_radius <= pos[2]) return true;
	if (floor_height / 2 - hole_size + _BALL_RADIUS / 2 > abs(pos[1]) && floor_width / 2 > abs(pos[0]) && ball_radius <= pos[2]) {
		//except center line hole
...
GLdouble *Collision::floor(Ball ball, GLdouble *vec) {
	if (floor_judge(ball.pos)) {
		vec[2] += -_GRAVITY; //reverse gravity of z vecter
		return vec;
	}
	return vec;
}
...
/* wall of in hole */
bool Collision::hole_lrside_judge(GLdouble pos[]) { //left and right wall of in hole
	if (floor_width / 2 - hole_size + _BALL_RADIUS / 2 > abs(pos[0])) return true;
	return false;
}
bool Collision::hole_tbside_judge(GLdouble pos[]) { //top and bottom wall of in hole
	if (floor_height / 2 - hole_size + _BALL_RADIUS / 2 > abs(pos[1])) return true;
	return false;
}
bool Collision::hole_center_judge(GLdouble pos[]) { //wall of center line hole
	if (abs(pos[1]) < hole_size * 2) {
		if (floor_width / 2 - abs(pos[0]) + abs(pos[1]) > hole_size - _BALL_RADIUS / 4) return true;
	}
	return false;
}
GLdouble *Collision::hole(Ball ball, GLdouble *vec) {
	for (size_t i = 0; i < 3; i++) vec[i] = 0.0;

	//true is wall reflection
	if (hole_lrside_judge(ball.pos)) vec[0] = -2.0 * ball.vec[0];
	if (hole_tbside_judge(ball.pos)) vec[1] = -2.0 * ball.vec[1];
...
/* wall of table */
bool Collision::wall_lrside_judge(GLdouble pos_x) {   //left and right wall
	if (floor_width / 2 - ball_radius < abs(pos_x)) return true;
	return false;
}
bool Collision::wall_tbside_judge(GLdouble pos_y) {   //top and bottom wall
	if (floor_height / 2 - ball_radius < abs(pos_y)) return true;
	return false;
}
GLdouble *Collision::wall(Ball ball, GLdouble *vec) {
	...
	//true is wall reflection
	if (wall_lrside_judge(ball.pos[0])) vec[0] = -2.0 * ball.vec[0];
	if (wall_tbside_judge(ball.pos[1])) vec[1] = -2.0 * ball.vec[1];
	return vec;
}
...
bool Collision::ball_judge(GLdouble target_pos[], GLdouble opponent_pos[]) {
	...
	for (size_t i = 0; i < 3; i++) distance += abs(target_pos[i] - opponent_pos[i]);

	//use distance and ball radius for judge
...
GLdouble *Collision::ball_not_collision_pos(Ball target, Ball opponent, GLdouble *pos) { //ƒRƒŠƒWƒ‡ƒ“‰ðœ
